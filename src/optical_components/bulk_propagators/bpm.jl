...
u_shift = propagate(u, shift_bpm)
u_diffraction = propagate(u, as_bpm)
...
function propagate!(u::ScalarField, p::BPM; u_saved = nothing)
    Nv = ndims(p.dn)
    n_slices = size(p.dn, Nv)
    ...
    for (dn, u_saved) in zip(@view(dn_slices[1:(end - 1)]), u_saved_slices)
        ...
        apply_dn_slice!(u, dn, p.kdz, Forward)
        propagate!(u, p.p_bpm)
    end
    copyto!(u_saved_slices[end], u.electric)
    apply_dn_slice!(u, dn_slices[end], p.kdz, Forward)
    propagate!(u, p.p_bpm_half)
    u
end
...
function backpropagate!(u::ScalarField,
    ...
    for (dn, ∂dn, u_saved) in zip(@view(dn_slices[end:-1:2]),
        ...
        apply_dn_slice!(u, dn, p.kdz, Backward)
        compute_dn_gradient!(∂dn, u_saved, u, p.kdz)
        backpropagate!(u, p.p_bpm)
    end
    apply_dn_slice!(u, dn_slices[1], p.kdz, Backward)
    compute_dn_gradient!(∂dn_slices[1], u_saved_slices[1], u, p.kdz)
    backpropagate!(u, p.p_bpm_half)
    u
end
